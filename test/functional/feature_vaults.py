...
from test_framework.script import OP_0, OP_1, OP_1NEGATE, OP_2, OP_2DUP, OP_3, OP_4, OP_5, OP_6, OP_7, OP_8, OP_ADD, OP_DROP, OP_DUP, OP_ELSE, OP_ENDIF, OP_EQUAL, OP_EQUALVERIFY, OP_GREATERTHAN, OP_IF, OP_INOUT_AMOUNT, OP_PICK, OP_ROLL, OP_SWAP, OP_VERIFY, CScript, OP_VAULT, OP_VAULT_RECOVER, CScriptNum
...
""")
    ...
    def test_batch_recovery(
        ...
        if recovery_auth.__class__ == NoRecoveryAuth:
            ...
            idx_error = {
                1: "Script failed an OP_EQUALVERIFY operation",
                2: "Script failed an OP_EQUALVERIFY operation",
            }
    ...
    def test_unauthed_recovery(self, node, wallet):
        ...
        self.assert_broadcast_tx(bad3, err_msg='Script failed an OP_EQUALVERIFY operation')
    ...
    def test_all_bad_recovery_types(self, recovery_auth, *recov_params, **recov_kwargs):
        ...
        for bb in [unique_bad,
            ...
            if bb in (BadRecoveryAmountLow, BadRecoveryAmountHigh) \
                ...
                # if the index is is valid (i.e. no INDEX_OUT_OF_BOUNDS), the amount will be wrong so
                # also check equal veirfy error message
                try_also = 'Script failed an OP_EQUALVERIFY operation'
...
class VaultSpec:
    ...
    def __init__(
        ...
        # need to add trigger_out_idx and add the input_indx to the witness stack so we can check that
        # if the sum of the amounts of triggerOut and revaultOut (if any) are not greater than or equal to the value of this input, 
        # script execution SHOULD fail and terminate immediately. 
        # This ensures that (at a minimum) the vaulted value for this input is carried through.
...
BadRevaultLowAmount = BadBehavior(
    "bad revault amount (low)", ("Bad revault", "Script failed an OP_EQUALVERIFY operation"))
...
BadRecoveryVoutIdx = BadBehavior("incorrect recovery vout idx", "Index out of bounds")
BadRecoveryVoutIdxNoExist = BadBehavior("nonexistent recovery vout idx", "Index out of bounds")

def indices_to_bitmap(indices, total_length):
    # Create a list of bits, starting all 0
    bits = [0] * total_length
    for idx in indices:
        bits[idx] = 1
    # Little-endian: least significant bit is index 0
    bits.reverse()
    # Convert bit list to integer
    bitmap_str = ''.join(str(b) for b in bits)
    return int(bitmap_str, 2)
...
def get_trigger_tx(
    ...
    for i, (vault, spec) in enumerate(vaults_to_spec.items()):
        ...
        #
        # the second check is removed here as we now add the revault_index to
        # to every input on the witness stack
        #if spec.revault_amount_sats and vault == spec.revault_input_vault:
        if spec.revault_amount_sats:
