...
from test_framework.script import LEAF_VERSION_TAPSCRIPT_64BIT, OP_1, OP_ENDIF, OP_EQUAL, OP_GREATERTHANOREQUAL, OP_IF, OP_IN_AMOUNT, OP_OUT_AMOUNT, OP_SUB, OP_SWAP, CScript, CScriptNum, CScriptOp, taproot_construct
...
class InOutAmountTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.tapscript_satisfy_test(script = CScript([OP_IN_AMOUNT, OP_SWAP, OP_OUT_AMOUNT, OP_SUB, encode(ONE_BTC), OP_GREATERTHANOREQUAL]), fundingAmount=2, spendingAmount=ONE_BTC, inputs = input1)
        
        # check we can't withdraw more than one BTC
        self.tapscript_satisfy_test(script = CScript([OP_OUT_AMOUNT, OP_SWAP, OP_IN_AMOUNT, OP_SUB, encode(ONE_BTC), OP_GREATERTHANOREQUAL]), fundingAmount=2, spendingAmount=ONE_BTC + 1, inputs = input1, fail='Script evaluated without error but finished with a false/empty top stack element')

        # check out of bounds input index
        self.tapscript_satisfy_test(script = CScript([OP_IN_AMOUNT, OP_SUB, encode(ONE_BTC), OP_GREATERTHANOREQUAL]), fundingAmount=2, spendingAmount=ONE_BTC + 1,
                                    inputs = [encodeWit(2)], fail='mandatory-script-verify-flag-failed (Index out of bounds)')
        self.tapscript_satisfy_test(script = CScript([OP_IN_AMOUNT, OP_SUB, encode(ONE_BTC), OP_GREATERTHANOREQUAL]), fundingAmount=2, spendingAmount=ONE_BTC + 1,
            inputs = [encodeWit(-1)], fail='mandatory-script-verify-flag-failed (Index out of bounds)')

        # check out of bounds output index
        self.tapscript_satisfy_test(script = CScript([OP_OUT_AMOUNT, OP_SUB, encode(ONE_BTC), OP_GREATERTHANOREQUAL]), fundingAmount=2, spendingAmount=ONE_BTC + 1,
                                    inputs = [encodeWit(1), encodeWit(4)], fail='mandatory-script-verify-flag-failed (Index out of bounds)')
        self.tapscript_satisfy_test(script = CScript([OP_OUT_AMOUNT, OP_SUB, encode(ONE_BTC), OP_GREATERTHANOREQUAL]), fundingAmount=2, spendingAmount=ONE_BTC + 1,
                                    inputs = [encodeWit(-1)], fail='mandatory-script-verify-flag-failed (Index out of bounds)')

        # check OP_0 (no input/output indices) pushes the amount 0 onto the stack
        self.tapscript_satisfy_test(script = CScript([OP_IN_AMOUNT, OP_OUT_AMOUNT, OP_EQUAL]), fundingAmount=2, spendingAmount=ONE_BTC + 1,
                inputs = [encodeWit(0), encodeWit(0)])
